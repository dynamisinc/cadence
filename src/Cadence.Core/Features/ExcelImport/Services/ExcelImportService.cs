    ...
    public async Task<ImportResultDto> ExecuteImportAsync(ExecuteImportRequestDto request)
    {
        ...
        foreach (var row in rowsToImport)
        {
            ...
            try
            {
                ...
                var mapResult = InjectRowMapper.MapRowToInject(inject, row.Values, session.Mappings,
                    phases, deliveryMethods, request.ExerciseId, exercise.OrganizationId,
                    request.CreateMissingPhases);
                warnings.AddRange(mapResult.Warnings);
                foreach (var newPhase in mapResult.NewPhases)
                {
