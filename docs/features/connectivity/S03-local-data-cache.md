...
### AC1: IndexedDB Setup
- [x] IndexedDB database created on application load
- [x] Database schema includes stores for: exercises, phases, injects, observations
- [x] Database handles versioning for schema migrations
- [x] Database errors are caught and logged gracefully

### AC2: Exercise Data Caching
- [x] Current exercise data cached when Conduct page loads
- [x] Exercise phases cached with exercise
- [x] Cache updated when real-time events received
- [x] Cache keyed by exercise ID for multi-exercise support

### AC3: Inject Data Caching
- [x] All injects for current exercise cached on load
- [x] Cache updated when inject status changes (fire/skip/reset)
- [x] Cache includes all inject fields needed for display
- [x] Cache indexed by exerciseId for efficient queries

### AC4: Observation Data Caching
- [x] All observations for current exercise cached on load
- [x] Cache updated when observations added/updated/deleted
- [x] Cache includes all observation fields needed for display
- [x] Cache indexed by exerciseId for efficient queries

### AC5: Offline Data Serving
- [x] When offline, data served from IndexedDB cache
- [x] Cached data used immediately, then updated from server when online
- [x] Stale data indicator shown when serving cached data offline
- [x] Cache timestamp tracked for freshness indication

### AC6: Cache Management
- [x] Cache cleared on user logout
- [x] Cache invalidated when newer data received from server
- [x] Old exercise data pruned after configurable period (7 days)
- [x] Cache size monitored to prevent storage issues
